...
void FadeDMG(UINT8 fadeout) {
	...
	for(i = 0; i != 4; ++i) {
		p = fadeout ? 3 - i : i;
		c = &colors[1 << 2];
		*pals[0] = PAL_DEF(FadeInOp(c[0], p), FadeInOp(c[1], p), FadeInOp(c[2], p), FadeInOp(c[3], p));
		//Sprites now use WHITE, LGREY & BLACK colors
		*pals[1] = PAL_DEF(FadeInOp(c[2], p), FadeInOp(c[0], p), FadeInOp(c[1], p), FadeInOp(c[3], p));
		*pals[2] = PAL_DEF(FadeInOp(c[2], p), FadeInOp(c[0], p), FadeInOp(c[1], p), FadeInOp(c[3], p));
		delay(50);
	}
}
